...
task test, "Run tests with all memory managers":
  for mm in ["orc", "arc", "refc"]:
    ...
    exec "nim c -r --mm:" & mm & " --threads:on --path:src -d:testing -d:allowSpinlockManagedRef tests/test.nim"
...
task testExamples, "Compile and run all example files":
  for file in listFiles("examples"):
    if file.endsWith(".nim"):
      echo "Testing: ", file
      exec "nim c -r --hints:off --threads:on --path:src -d:allowSpinlockManagedRef " & file
