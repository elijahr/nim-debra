# tests/t_signal.nim
##
## SIGUSR1 handler regression tests. POSIX only — on Windows the
## neutralization protocol uses SuspendThread/ResumeThread and there
## is no async handler to test here. The cross-platform equivalent
## (`neutralizeRemoteSlot`) is exercised end-to-end by `t_neutralize`.
